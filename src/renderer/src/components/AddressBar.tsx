...
export function AddressBar({ pageTitle }: { pageTitle?: ReactNode }) {
  ...
  if (editing) {
    return (
      <input
        ref={inputRef}
        className="border-foreground/30 bg-background text-foreground flex-1 select-text rounded-md border px-2 py-1.25 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 [-webkit-app-region:no-drag]"
        value={value}
