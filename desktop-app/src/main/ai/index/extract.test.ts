...
import { extractText, isExtractable, MAX_BYTES } from './extract';

describe('isExtractable', () => {
  it('accepts text/code/data extensions and rejects binaries', () => {
    for (const p of ['/a/notes.md', '/a/main.ts', '/a/data.json', '/a/q.SQL']) {
      expect(isExtractable(p)).toBe(true);
    }
    for (const p of ['/a/photo.png', '/a/clip.mp4', '/a/archive.zip', '/a/report.docx', '/a/noext']) {
