...
// [col, row, ghost] on a 0..2 grid — 6 active nodes form a reversed "F",
// 3 ghost nodes (bottom-right) hold the grid. Matches the desktop <Mark>.
...
export function Wordmark({ className }: { className?: string }) {
  return (
    <span className={cn("inline-flex items-baseline leading-none", className)}>
      ...
      <span className="font-mono font-normal text-azure">DOS</span>
    </span>
  );
}
...
export function Logo({ className }: { className?: string }) {
  return (
    <span className={cn("inline-flex items-center gap-2", className)}>
      <Mark className="size-[1.1em] text-azure" />
      <Wordmark />
    </span>
  );
}
