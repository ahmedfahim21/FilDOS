  ...
  return (
    <span
      className={cn(
        ...
        cell.good ? "font-medium text-mint" : "text-muted-foreground"
      )}
    >
      {cell.mark === "yes" && <Check className="size-4" strokeWidth={2.75} />}
      {cell.mark === "no" && (
        <X className={cn("size-4", !cell.good && "text-foreground/25")} strokeWidth={2.75} />
      )}
      {cell.text}
    </span>
  );
 ...
 */
const RAISED = "border-x border-foreground/8 bg-card";

export function LandingCompare() {
  return (
    <section id="compare" className="scroll-mt-16 bg-muted/60 py-16 sm:py-24">
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr>
              ...
              <th className="px-3 py-4 text-center text-sm font-medium text-muted-foreground">
                Other AI apps
              </th>
            </tr>
          </thead>
          <tbody>
            {ROWS.map(({ label, fildos, cloud }, i) => {
              ...
              const divider = !last && "border-b border-foreground/8";
              return (
                <tr key={label}>
                  <td className={cn("py-3.5 pr-4 text-sm text-foreground/80", divider)}>{label}</td>
