/**
 * Installs a git pre-commit hook that runs lint-staged for this project.
 * It resolves the *real* hooks directory via `git rev-parse` (rather than
 * assuming the package sits at the git root) and points the hook back at the
 * project dir, so it works whether the app is the repo root or nested.
 *
...
try {
  ...
  const hook = [
    '#!/usr/bin/env sh',
    '# Managed by scripts/install-hooks.mjs — do not edit.',
